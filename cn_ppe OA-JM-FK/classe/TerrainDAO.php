...
public function create($_o) {
	...
	try{
		$requete_create=$connexion->prepare("insert into terrain (id,latitude,longitude,superficie,plan,prix) values (:id,:lat,:lon,:s,:pl,:pr)"); // on prépare notre requête
...
public function update($_lat,$_lon,$_sup,$_plan,$_prix,$_id) {
	...
	try{
		$requete_update=$connexion->prepare("Update terrain set latitude=".$_lat.", longitude=".$_lon.", superficie=".$_sup.", plan='".$_plan."', prix=".$_prix." where id=".$_id); // on prépare notre requête
